...
export default class ProductContent extends Component {
  ...
  checkCartDuplicates() {
    if (!(_.isEqual(this.props.cartItems, []))) {
      this.props.cartItems.forEach((item, idx) => {
        const data = this.getCurrentData();
        if ((item.productId === data.productId) && (_.isEqual(item.attributes, data.attributes))) {
  ...
  getCurrentData() {
    ...
    const itemFixedPrice = Number(
      this.props.currentProduct.prices[this.props.currencySymbol[0]].amount
    );
  ...
  cartStateHandler() {
    ...
    const data = this.getCurrentData();
    this.props.cartItemsHandler(data);
  ...
  render() {
    ...
    const printImageThumbnails = this.props.currentProduct.gallery.map(
      (thumbnail, idx) => {
        const result = (
          <div
            key={idx}
    ...
    // Handling Attributes...
    const attributes = this.props.currentProduct.attributes.map(
      (attribute, index) => {
          ...
          const attributeValueTemplate = (
            ...
            >
              <div
                ...
                style={{
                  background:
                    value.value === "#FFFFFF" ? "#D3D2D5" : value.value,
                }}
              ></div>
            </div>
          );
      ...
      <div className="product-view-container">
        ...
        <div className="product-view-image">
          <div className="product-view-image-div">
            <img
              src={this.props.currentProduct.gallery[this.state.thumbnailId]}
          ...
          <div
            ...
            style={{
              display: this.props.currentProduct.inStock ? "none" : "block",
            }}
          >
        ...
        <div className="product-view-details-wrapper">
          <div className="product-view-details-brand">
            {this.props.currentProduct.brand}
          </div>
          ...
          <div className="product-view-details-price-wrapper">
            ...
            <div className="price">{`${this.props.currencySymbol[1]}${
              this.props.currentProduct.prices[this.props.currencySymbol[0]]
                .amount
            }`}</div>
          </div>
            ...
            <button
              onClick={() => {
                if (this.props.currentProduct.inStock)
                  this.checkCartDuplicates();
              }}
              style={{
                display: this.props.currentProduct.inStock ? "block" : "none",
              }}
            >
              ADD TO CART
            </button>
